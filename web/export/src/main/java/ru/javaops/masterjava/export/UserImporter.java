        ...
        return new Callable<List<FailedEmail>>() {
                ...
                while (processor.doUntil(XMLEvent.START_ELEMENT, "User")) {
                    ...
                    if (city == null) {
                       ...
                       if (!groups.keySet().containsAll(groupNames)) {
                           failed.add(new FailedEmail(email, "One of groups from '" + groupRefs + "' is not present in db"));
                       } else {
            ...
            private ChunkFuture submit(List<ChunkItem> chunk) {
                ...
                ChunkFuture chunkFuture = new ChunkFuture(users,
                    ...
                    List<User> alreadyPresent = userDao.insertAndGetConflictEmails(users);
                    Set<Integer> alreadyPresentIds = StreamEx.of(alreadyPresent).map(User::getId).toSet();
                    userGroupDao.insertBatch(StreamEx.of(chunk).flatMap(ChunkItem::getUserGroups)
                    .filter(ug -> !alreadyPresentIds.contains(ug.getUserId()))
                    .toList());
                    return StreamEx.of(alreadyPresent).map(User::getEmail).toList();
                        })
                );
